...
class Adafruit : public QSerialPort
{
    Q_OBJECT
public:

    // Constructor
    explicit Adafruit(QWidget *parent = 0) : QSerialPort(parent)
    {
    ...
    {
        // init Answer
        QString answer;// = QString(readAll());

        // get answer in a single string
        while (waitForReadyRead (5) && !(answer.endsWith("!")) )
            answer.append(QString(readAll()));



       /* bool end = answer.endsWith("!");
        // get answer in a single string
        while (!end){
